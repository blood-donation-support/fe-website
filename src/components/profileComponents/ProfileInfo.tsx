...
const ProfileInfo: React.FC = () => {
  ...
  useEffect(() => {
    fetchBloodGroups().then(setBloodGroups).catch(() => {});
  }, []);
    ...
    if (profile && bloodGroups.length) {
      ...
      const defaultBloodGroupId = (() => {
        ...
        if (profile.blood_group && profile.blood_group.name) {
          const found = bloodGroups.find(bg => bg.name === profile.blood_group?.name);
          return found?._id || "";
        }
        return "";
      })();
  ...
  if (!profile) {
    return (
      <div className="flex justify-center items-center py-20">
        <span className="text-gray-400 text-lg">Đang tải thông tin cá nhân...</span>
      </div>
    );
  }
  ...
  const checkDirty = (current: any) => {
    ...
    return (
      ...
      current.date_of_birth !== dayjs(profile.date_of_birth).format("YYYY-MM-DD") ||
  ...
  const handleAvatarUpload = async (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!file) return;
    setTimeout(() => {
      setFormData((old: any) => ({ ...old, avatar_url: URL.createObjectURL(file) }));
      setIsDirty(true);
...
console.log("bloodGroups:", bloodGroups);console.log("blood_group_id value:", formData.blood_group_id);
  return (
      ...
      >
        ...
        <div className="pl-24 grid grid-cols-1 md:grid-cols-2 gap-x-12 gap-y-6">
          ...
          <TextField
            ...
            onChange={(e) => handleFieldChange("blood_group_id", e.target.value)}
            fullWidth
            margin="normal"
            InputProps={{ readOnly: !isEdit }}
            disabled={!isEdit}
          ...
          >
            {bloodGroups.map(bg => (
              <MenuItem key={bg._id} value={bg._id}>{bg.name}</MenuItem>
            ))}
          </TextField>
          {/* Chỉ view, không edit */}
          <TextField
          ...
          <TextField
            ...
            value={profile.number_of_donations >= 0 ? `${profile.number_of_donations} lần` : ""}
            fullWidth
            margin="normal"
            InputProps={{ readOnly: true }}
            disabled
          ...
          <TextField
            ...
            value={profile.number_of_requests >= 0 ? `${profile.number_of_requests} lần` : ""}
            fullWidth
            margin="normal"
            InputProps={{ readOnly: true }}
            disabled
