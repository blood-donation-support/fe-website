...
export default function ServiceCard({ number, title, description, links, icon, inview }: ServiceCardProps) {
  ...
  return (
    <motion.div
        initial={{ opacity: 0,scale:1, y: -150, x: 0 ,height:'60%'}}
        animate={inview?{ opacity: 1,scale:1, y: 0, x: 0 ,height:'100%' }:{ opacity: 0,scale:1, y: -150, x: 0,height:'60%' }}
        ...
        >
            ...
            className="absolute z-0 left-7 top-6 text-[4rem] font-extrabold text-black  select-none pointer-events-none leading-none">
            ...
            {/* <span
                ...
                top-[4.94rem]    // nên để trùng với top của số
                h-0.5     // chiều cao gạch (tùy chiều cao content)
                w-12      // độ dày gạch
            ...
            transition={{ duration: 1, delay: 0 }}
            className="relative z-10 flex-1 flex flex-col bg-white mt-24">
                <span className="font-bold text-[0.8rem] flex items-center gap-2 mb-1">
                ...
                className="text-gray-500 text-[0.8rem] mb-8 block">
                {description}
                </motion.span>
